...
这里结合我们初代的推荐系统，彼时通过共享内存的方式支持索引，来达到共享数据的目的。一个cpu挂一个进程，固然代码好写也高效，但经常出现死锁，不就是这个问题嘛。所以后来，大家意识到这种设计的不足，才做了服务化的改进。
...
首先，event loop没有标准的实现，大多为开发者自己实现。其次，第三方库不一定能很好的适应event loop，比如有时候需要线程来做一些串并转换，并且c++也没有标准的线程库。我理解chenshuo想表达
的是，由于标准的确实，第三库的实现更多的考虑自己即可，所有通过自己开线程的方式来实现。

7. 什么是线程池大小的阻抗匹配原则？

给出线程池大小的经验公式： T = C/P，其中C表示CPU数目，或者分配给这项任务的CPU数目，P代表密集计算所占的时间比重。

8. 除了Reactor + thread pool，还有别的non-trivial多线程编程模型嘛？

有，Procator。Proactor模式依赖操作系统或库来高效地调度这些子任务，每个子任务都不会阻塞，因此能用比较少的线程达到很高的IO并发度。但是这是异步非阻塞的IO model，代码难写，不易理解。
因此，在现代RPC框架下，都支持了协程来简化代码开发难度。

9. 模式2和模式3a该如何取舍？

首先，在目前多核时代，模式2和模式3肯定是主流的编程模型。至于这2者的取舍，前文也提到了什么时候用模式2，和模式3.此处，chenshuo又给出了一个选择依据：
在其他条件相同的情况下，可以根据工作集（work set）的大小来取舍。工作集是指服务程序响应一次请求所访问的内存大小。如果工作集较大，那么就用多线程，避免CPU cache 换入换出，影响性能；否
则，就用单线程多进程，享受单线程编程的便利。